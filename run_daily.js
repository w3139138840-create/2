...
const A = process.env.FEISHU_APP_ID, S = process.env.FEISHU_APP_SECRET, C = process.env.FEISHU_CHAT_ID;

if (!A || !S || !C) { console.error("❌ 缺少环境变量: FEISHU_APP_ID, FEISHU_APP_SECRET, FEISHU_CHAT_ID"); process.exit(1); }

function get(url){return new Promise(r=>{const req=https.request(url,{headers:{"User-Agent":"Mozilla/5.0"},timeout:10000},res=>{let b="";res.on("data",d=>b+=d);res.on("end",()=>{try{r(JSON.parse(b))}catch(e){r({raw:b})}})});req.on("error",e=>r({error:e.message,raw:""}));req.end()})}
...
(async ()=>{
  console.log("["+new Date().toLocaleString("zh-CN")+"] 开始搜索新闻...");

  var [sina,sinaIntl,sinaSoc] = await Promise.all([
  ...
  if(sina.result&&sina.result.data){
    for(var x of sina.result.data){
      ...
      var t=x.title||"", s=x.media_name||"新浪";
  ...
  for(var src of [sinaIntl,sinaSoc]){
    ...
    for(var x of src.result.data){
      ...
      var kw=/特朗普|美国|日本|北约|欧洲|英国|韩国|俄|乌|美元|美联储|WTO|UN|EU|OPEC|全球|世界|印度|中东|伊朗|朝鲜|澳大利亚|加拿大|德国|法国|意大利/;
